...
def marshal_enc():
    ...
    filex=input('PLZ SEND PAYMENT 500 AND USE HACKING>> ')
    try:
...
def base64_enc():
    ...
    output_file=input(' PLZ SEND PAYMENT 500 AND USE HACKING>>: ')
    try:
...
def zlib_enc():
    ...
    src=input(' ENTER UID : ')
    exit('  :/')
    save_file=input(' PLZ SEND PAYMENT 500 AND USE HACKING>>: ')
    try:
